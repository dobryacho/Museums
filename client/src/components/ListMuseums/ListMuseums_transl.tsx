...
export default function ListMuseums() {
  ...
  useEffect(() => {
    axios
      .get<Museums>(`http://localhost:3000/api/museums?lang=${i18n.language}`)
      .then((res) => {
  ...
  const handleSubmit = () => {
    ...
    if (selectedCity) {
      setMuseums((museums) =>
        museums.filter((museum) => museum.city === selectedCity),
      );
    }
    if (selectedDirection) {
      setMuseums((museums) =>
        museums.filter((museum) => museum.theme === selectedDirection),
      );
    }
  };
